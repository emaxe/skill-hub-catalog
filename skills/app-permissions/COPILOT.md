...
Проверка прав в проекте централизована в Pinia-сторе `useAppPermissions` (Shared.UI). Права приходят с бэкенда и сохраняются в `useConfigStore().permissions`.

## Источник и инициализация

- **Стор:** `Shared.UI/stores/commonPermissions.js` — экспортирует `useAppPermissions`.
- **Импорт:** `import { useAppPermissions } from '@rt/shared/stores/commonPermissions'`.
- **Данные прав:** хранятся в `useConfigStore().permissions` (массив строк).
